...
| [prevent-committing-secrets](./skills/prevent-committing-secrets/SKILL.md) | skill | Claude, Codex | Block a credential at commit time with a gitleaks pre-commit hook, and cover new/cloned repos automatically via `init.templateDir`: the four paths that skip the hook (`--no-verify`, merge commits, replayed commits, `core.hooksPath`), why a staged-diff scan is not a history scan, and GitHub push protection as the half that cannot be bypassed locally |
