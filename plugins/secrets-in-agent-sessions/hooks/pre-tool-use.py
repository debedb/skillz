...
def reason_text(hits):
    ...
    lines += [
        ...
        "",
        "Reference it from the environment so the value never reaches the",
        "RECORDED command string:",
        "",
        "    KEY=\"$(fetch-it)\" sh -c 'curl -H \"X-Api-Key: $KEY\" https://...'",
        "",
        "That closes the persistent copies. It does NOT close argv -- the inner",
        "shell expands $KEY before exec'ing curl. To close both, pass it on",
        "stdin, which never reaches the argument vector:",
        "",
        "    KEY=\"$(fetch-it)\" sh -c \\",
